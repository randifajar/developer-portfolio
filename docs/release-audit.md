...
| Q10 — `remoteAvailability` rename | **Closed 2026-08-25: renamed to `availabilityStatement`.** Twice-deferred is a decision by attrition, so it was made deliberately. 8 occurrences across 6 files; no rendered text changed. Both guards break-tested after the rename — removing the field fails typecheck, an empty value fails content validation with `Too small: expected string to have >=1 characters` |
